...
import { IVideoData } from '../types/VideoData';
import iso8601 from 'iso8601-duration';

const getVideoIdFromUrl = (url: string) => {
    const urlSplit = url.split(/(vi\/|v=|\/v\/|youtu\.be\/|\/embed\/)/);
    return urlSplit[2]?.split(/[^0-9a-z_\-]/i)[0] ?? urlSplit[0];
};

const getVideoData = async (id: string): Promise<IVideoData | null> => {
    try {
        const response = await google
            .youtube('v3')
            .videos.list({ part: ['snippet', 'contentDetails'], id: [id], auth: process.env.YOUTUBE_DATA_API_KEY });
